...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const initializeAuth = async () => {
    try {
      ...
      if (storedToken && storedAdmin) {
        ...
        try {
          const response = await fetch('https://vtu-application.onrender.com/api/admin/auth/verify', {
            method: 'GET',
            headers: {
  ...
  const refreshToken = async (): Promise<boolean> => {
    ...
    try {
      const response = await fetch('https://vtu-application.onrender.com/api/admin/auth/refresh', {
        method: 'POST',
        headers: {
