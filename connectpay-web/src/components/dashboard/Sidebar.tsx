...
const Sidebar = ({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (isMobile && isOpen && sidebarRef.current && !sidebarRef.current.contains(event.target as Node)) {
        const isBurger = (event.target as HTMLElement).closest('.dh-burger');
        if (!isBurger) setIsOpen(false);
      }
    };
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape' && isOpen && isMobile) setIsOpen(false);
    };
  ...
  const toggleMenu = (menuId: string) => {
    ...
    setExpandedMenus((prev: any) => ({ ...prev, [menuId]: !prev[menuId] }));
  };
  ...
  const menuItems = [
    ...
    {
      id: 'other-services', label: 'Other Services', icon: Menu,
      submenu: [
    ...
    {
      id: 'self-service', label: 'Self Service', icon: HelpCircle,
      submenu: [
        { label: 'Verify Data Purchase', path: '/dashboard/verify-data-purchase', icon: CheckCircle },
      ],
    },
    { id: 'mobile-app',   label: 'Download Mobile App',  icon: Smartphone, path: '/dashboard/mobile-app' },
    ...
    { id: 'wallet',  label: 'Wallet Summary', icon: Wallet,    path: '/dashboard/wallet-summary' },
    { id: 'profile', label: 'Profile',        icon: User,      path: '/dashboard/profile' },
    { id: 'support', label: 'Support',        icon: HelpCircle, path: '/dashboard/need-help' },
          ...
          {!isCollapsed ? (
            <div className="user-info">
              <h3 className="user-name">
                Welcome, {contextUser?.name?.split(' ')[0] || 'User'}
              </h3>
              <div className="user-wallet">
                <Wallet size={14} />
                <span>
                  e-Wallet: ₦{balance.toLocaleString('en-US', {
