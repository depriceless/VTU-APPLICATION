  ...
  return (
    <div style={{ padding: '16px 24px', maxWidth: 1400, margin: '0 auto' }}>
      ...
      <div style={{ background: 'white', borderRadius: 12, padding: '20px 24px', marginBottom: 24, border: '1px solid #e5e7eb' }}>
        <div style={{ height: 18, width: '60%', background: '#e5e7eb', borderRadius: 6, margin: '0 auto', animation: 'pulse 1.5s infinite' }} />
      </div>
      <div style={{ background: 'white', borderRadius: 12, border: '1px solid #e5e7eb', overflow: 'hidden' }}>
        <div style={{ background: '#f9fafb', padding: '14px 12px', borderBottom: '2px solid #e5e7eb', display: 'flex', gap: 12 }}>
          {[30, 80, 70, 120, 60, 60, 100, 140, 100, 70, 50].map((w, i) => (
            <div key={i} style={{ height: 13, width: w, background: '#e5e7eb', borderRadius: 4, flexShrink: 0, animation: 'pulse 1.5s infinite' }} />
          ))}
        </div>
        {[0, 1, 2, 3, 4].map(i => (
          <div key={i} style={{ padding: '14px 12px', borderBottom: '1px solid #f3f4f6', display: 'flex', gap: 12, alignItems: 'center' }}>
            {[30, 80, 70, 120, 60, 60, 100, 140, 100, 60, 40].map((w, j) => (
