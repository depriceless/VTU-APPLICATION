...
export default function BuyNeco() {
  ...
  const fetchEducationPackages = async () => {
    ...
    try {
        ...
        if (packagesData.neco && Array.isArray(packagesData.neco)) {
          packagesData.neco.forEach((pkg: any) => {
            transformedCards.push({
              ...
              name: pkg.name,
              code: pkg.code,
              price: pkg.price, // ✅ Use price as-is (backend already added ₦50)
              description: pkg.description || pkg.name,
              examBody: 'NECO', // ✅ FIXED: Use uppercase to match backend requirements
      ...
      alert('Failed to load NECO packages. Please refresh the page.');
    } finally {
  ...
  const handleBuyEducation = async () => {
    ...
    try {
      const response = await apiClient.post('/purchase', {
        type: 'education',
        provider: selectedExam?.examBody.toUpperCase(), // ✅ FIXED: Convert to uppercase
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        <h1 className="page-title">BUY NECO PINS</h1>
        <div className="breadcrumb">
          <span className="breadcrumb-link">Dashboard</span>
          <span className="breadcrumb-separator">/</span>
          <span className="breadcrumb-current">NECO</span>
        </div>
      </div>
      ...
      {currentStep === 1 ? (
        <div className="card">
          ...
          <div className="form-section">
            <label className="section-label">Select NECO Exam Card</label>
            {isLoadingPackages ? (
              <div className="loading-plans">
                <div className="spinner-small"></div>
                <span>Loading NECO packages...</span>
              </div>
            ) : examCards.length === 0 ? (
              <div className="empty-state">No NECO packages available</div>
            ) : (
                ...
                <div className="pins-section">
                  ...
                  {successData.pins.map((pinData: any, index: number) => (
                    <div key={index} className="pin-card">
                      ...
                      <div className="pin-row">
                        <span>Serial:</span>
                        <span className="pin-serial">{pinData.serial || 'N/A'}</span>
                      </div>
                    </div>
                  ))}
                </div>
