...
function CustomerVerifiedModal({ isOpen, onConfirm, onClose, customerName, customerAddress, meterNumber, providerName }) {
  ...
  return (
    <>
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 9998, backdropFilter: 'blur(3px)', animation: 'cvFadeIn 0.2s ease' }} />
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 360, maxWidth: 'calc(100vw - 32px)', background: 'white', borderRadius: 20, zIndex: 9999, boxShadow: '0 32px 64px rgba(0,0,0,0.18), 0 0 0 1px rgba(0,0,0,0.04)', animation: 'cvPopIn 0.25s cubic-bezier(0.34, 1.56, 0.64, 1)', overflow: 'hidden' }}>
        <div style={{ background: 'linear-gradient(135deg, #16a34a 0%, #15803d 100%)', padding: '24px 20px 20px', textAlign: 'center', position: 'relative' }}>
          ...
          <div style={{ width: 56, height: 56, borderRadius: '50%', background: 'rgba(255,255,255,0.2)', border: '2px solid rgba(255,255,255,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px', animation: 'cvScaleIn 0.3s cubic-bezier(0.34, 1.56, 0.64, 1) 0.1s both' }}>
      ...
      <style>{`
        ...
        @keyframes cvPopIn { from { opacity: 0; transform: translate(-50%, -48%) scale(0.92) } to { opacity: 1; transform: translate(-50%, -50%) scale(1) } }
        @keyframes cvScaleIn { from { transform: scale(0.5); opacity: 0 } to { transform: scale(1); opacity: 1 } }
      `}</style>
    </>
  );
}
...
function ProviderBottomSheet({ isOpen, onClose, providers, selectedProvider, onSelect }) {
  ...
  const sheetStyle = isDesktop ? {
    ...
    borderRadius: 16, animation: 'fadeScaleIn 0.2s ease', boxShadow: '0 24px 48px rgba(0,0,0,0.2)'
  } : {
    position: 'fixed', left: 0, right: 0, bottom: 0, maxHeight: '85vh',
    borderRadius: '20px 20px 0 0', animation: 'slideUp 0.3s cubic-bezier(0.32, 0.72, 0, 1)',
    ...
    <>
      ...
      <div style={{ background: 'white', zIndex: 9999, display: 'flex', flexDirection: 'column', ...sheetStyle }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: isDesktop ? '20px 20px 12px' : '8px 20px 12px', borderBottom: '1px solid #f3f4f6' }}>
          ...
          <button onClick={onClose} style={{ width: 34, height: 34, borderRadius: '50%', background: '#f3f4f6', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#6b7280', flexShrink: 0 }}><X size={16} /></button>
        </div>
...
function MeterTypeBottomSheet({ isOpen, onClose, meterTypes, selectedMeterType, onSelect }) {
  ...
  if (!isOpen) return null;
  const sheetStyle = isDesktop
    ? { position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 420, maxWidth: '90vw', borderRadius: 16, animation: 'fadeScaleIn 0.2s ease', boxShadow: '0 24px 48px rgba(0,0,0,0.2)' }
    : { position: 'fixed', left: 0, right: 0, bottom: 0, borderRadius: '20px 20px 0 0', animation: 'slideUp 0.3s cubic-bezier(0.32, 0.72, 0, 1)', boxShadow: '0 -8px 32px rgba(0,0,0,0.15)' };
  return (
    <>
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 9998, animation: 'fadeIn 0.2s ease' }} />
      <div style={{ background: 'white', zIndex: 9999, display: 'flex', flexDirection: 'column', ...sheetStyle }}>
...
export default function BuyElectricityPage() {
  ...
  useEffect(() => {
    if (isMeterNumberValid && selectedProvider && selectedMeterType) {
      ...
      setCustomerName(''); setCustomerAddress(''); setCustomerConfirmed(false); setMeterError('');
    }
  ...
  useEffect(() => {
    if (showPinModal) { setPin(''); setPinError(''); setTimeout(() => pinInputRef.current?.focus(), 300); }
  }, [showPinModal]);
  ...
  const fetchElectricityProviders = async () => {
    try {
      ...
      } else { setElectricityProviders(defaultProviders); }
    } catch { setElectricityProviders(defaultProviders); }
  };
  ...
  const validateMeter = async () => {
    ...
    setMeterError(''); setCustomerName(''); setCustomerAddress(''); setCustomerConfirmed(false);

    const retryMsgTimer = setTimeout(() => setValidatingMessage('Taking longer than usual, still verifying...'), 20000);

    try {
      const response = await apiClient.post('/electricity/validate-meter', {
        meterNumber, provider: selectedProvider.id, meterType: selectedMeterType.id,
  ...
  const handleCustomerConfirm = () => { setCustomerConfirmed(true); setShowCustomerModal(false); };
  const handleCustomerModalClose = () => {
    setShowCustomerModal(false); setCustomerName(''); setCustomerAddress('');
    setCustomerConfirmed(false); setMeterNumber(''); setMeterError('');
  };
  ...
  const fetchUserBalance = async () => {
    ...
    try {
      ...
      if (response.data?.success && isMountedRef.current) setBalance(extractBalance(response.data.balance));
    } catch {}
    finally { if (isMountedRef.current) { setIsLoadingBalance(false); setIsLoading(false); } }
  };
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        <div style={{ height: 24, width: 150, background: '#e5e7eb', borderRadius: 6, marginBottom: 10, animation: 'pulse 1.5s infinite' }} />
        <div style={{ height: 14, width: 110, background: '#f3f4f6', borderRadius: 6, animation: 'pulse 1.5s infinite' }} />
      </div>
      <div style={{ background: 'white', borderRadius: 12, padding: 20, maxWidth: 600, margin: '0 auto', border: '1px solid #e5e7eb' }}>
        <div style={{ height: 48, background: '#f3f4f6', borderRadius: 8, marginBottom: 20, animation: 'pulse 1.5s infinite' }} />
        {[0, 1, 2, 3].map(i => (
          <div key={i} style={{ marginBottom: 18 }}>
            <div style={{ height: 13, width: 120, background: '#e5e7eb', borderRadius: 4, marginBottom: 7, animation: 'pulse 1.5s infinite' }} />
            <div style={{ height: 44, background: '#f3f4f6', borderRadius: 8, animation: 'pulse 1.5s infinite' }} />
          </div>
        ))}
        <div style={{ height: 48, background: '#e5e7eb', borderRadius: 8, marginTop: 8, animation: 'pulse 1.5s infinite' }} />
      </div>
  ...
  return (
    <div className="page-container">
              ...
              <div className="form-group">
                ...
                <div className="input-container">
                  <input type="text" value={meterNumber}
                    onChange={e => { setMeterNumber(e.target.value.replace(/\D/g, '')); setCustomerName(''); setCustomerAddress(''); setCustomerConfirmed(false); setMeterError(''); }}
                    placeholder="Enter meter number (10+ digits)" maxLength={13}
                ...
                {isValidatingMeter && (
                  <div className="validating-msg"><div className="spinner-tiny" />{validatingMessage}</div>
                )}
                {meterError && (
                ...
                {customerConfirmed && customerName && (
                  <div className="customer-confirmed-chip">
                    ...
                    <button onClick={() => setShowCustomerModal(true)} style={{ marginLeft: 'auto', fontSize: 11, color: '#16a34a', background: 'none', border: 'none', cursor: 'pointer', padding: '0 2px', fontWeight: 600, textDecoration: 'underline', flexShrink: 0 }}>
                      View
                    </button>
                  </div>
                )}
            ...
            <>
              <div className="form-group">
                ...
                <input type="number" value={amount} onChange={e => setAmount(e.target.value)}
                  placeholder={`Min. ₦${minAmount.toLocaleString()}`} min={minAmount} max="100000" className="text-input" />
      ...
      <ProviderBottomSheet
        ...
        onSelect={(p) => {
          setSelectedProvider(p); setSelectedMeterType(null); setMeterNumber('');
          setCustomerName(''); setCustomerAddress(''); setCustomerConfirmed(false); setMeterError('');
        }}
      />
